...
"""
    create_magneticfield_itps(brxp, snap, itp_type, itp_bc, filename)
Create interpolation objects for the magnetic field components and save them as
a JLD2-file.
## Keyword arguments
- `units::String`: Units of the magnetic field components. Default is "si".
- `destagger::Bool`: Whether to destagger the magnetic field components. Default
  is `true`.
"""
function create_magneticfield_itps(
    brxp::BifrostExperiment,
    snap::Integer,
    itp_type::Interpolations.InterpolationType,
    itp_bc::Union{
    ...
    bitp = Vector{AbstractInterpolation}(undef, 3)
    for (var, i) in zip(("bx", "by", "bz"), 1:3)
        bitp[i], _ = get_br_var_interpolator(
            brxp,
            snap,
            var,
            itp_type=itp_type,
    ...
    @save string(filename, "_B.jld2") bitp
end
